 ...
 * 13. Convert the text formatting of TG CRM to HTML
 * 14. Remove the text formatting of TG CRM
 * 15. Newsletter
...
function sb_email_create($recipient_id, $sender_name, $sender_profile_image, $message, $attachments = [], $department = false, $conversation_id = false, $cc = false) {
    $recipient = false;
    $recipient_name = '';
    $recipient_email = '';
...
function sb_email_create_content($subject, $body, $attachments, $replacements) {
    ...
    $subject = str_replace(['{recipient_name}', '{sender_name}', '{conversation_id}'], [$replacements['recipient_name'], sb_isset($replacements, 'sender_name'), sb_isset($replacements, 'conversation_id')], $subject);
    $body = str_replace(['{conversation_url_parameter}', '{recipient_name}', '{sender_name}', '{sender_profile_image}', '{message}', '{attachments}', '{conversation_link}', '{conversation_id}'], ['conversation_url_parameter' => sb_isset($replacements, 'conversation_url_parameter', ''), $replacements['recipient_name'], sb_isset($replacements, 'sender_name'), str_replace('user.svg', 'user.png', sb_isset($replacements, 'sender_profile_image')), $replacements['message'], sb_email_attachments_code($attachments), (sb_is_cloud() ? CLOUD_URL : SB_URL . '/admin.php') . (isset($replacements['conversation_id']) ? '?conversation=' . $replacements['conversation_id'] : ''), sb_isset($replacements, 'conversation_id')], $body);
    return [$subject, $body];
}
                            ...
                            if (!in_array($id, $history) && !in_array($id, $history_new)) {
                                ...
                                if ($sender && ($follow_up || !sb_is_agent($sender))) {
                                    ...
                                    for ($i = 0; $i < count($attachments); $i++) {
                                        ...
                                        if (sb_is_allowed_extension(pathinfo($file_name, PATHINFO_EXTENSION))) {
                                    ...
                                    if (!empty($message)) {
                                        ...
                                        if (!$agent) {
                                            if (defined('SB_DIALOGFLOW') && sb_get_setting('dialogflow-email-piping')) {
                                                ...
                                                sb_db_query('DELETE FROM sb_messages WHERE conversation_id = ' . $conversation_id . ' AND payload LIKE "%NO_MATCH%" AND creation_time < "' . sb_gmt_now(-60) . '" ORDER BY id DESC LIMIT 1');
...
function sb_email_piping_suffix($conversation_id) {
    return $conversation_id && sb_email_piping_is_active() ? (' | SB' . $conversation_id . '-' . rand(100, 9999)) : '';
}

function sb_email_piping_is_active() {
