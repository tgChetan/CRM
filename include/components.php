    ...
    <div class="sb-profile-box sb-lightbox">
        ...
        <div class="sb-main sb-scroll-area">
            ...
            <div>
				...
				<div class="sb-title" style="margin-top:15px;margin-bottom:5px">
                    <?php sb_e('User Communities') ?>
                </div>
                <div id="user_communities"></div>
            </div>
        </div>
    </div>
<?php } ?>
...
function sb_direct_message_box() { ?>
    ...
    ?>
    <div class="sb-editor<?php echo !$enabled[0] || !$enabled[1] ? ' sb-disabled-' . (!$enabled[0] && !$enabled[1] ? '2' : '1') : '' ?>" style="width:550px !important">
	
            <div class="sb-setting sb-type-textarea sb-textarea" >
       ...
       // $code .= '<div class="sb-btn-saved-replies" data-sb-tooltip="' . sb_('Add a saved reply') . '"></div>';
        ...
        echo $code . '</div><div class="sb-icon-send sb-submit" data-sb-tooltip="' . sb_('Send message') . '" style="display:none"></div><i class="sb-loader"></i></div>';
        ?>
...
<?php
function sb_routing_select($exclude_id = false) {
    $agents = sb_db_get('SELECT id, first_name, last_name 
        FROM sb_users 
        WHERE (user_type = "agent" OR user_type = "admin")' 
        . ($exclude_id ? (' AND id <> ' . sb_db_escape($exclude_id)) : '') . 
        ' ORDER BY first_name ASC, last_name ASC', false);

    $code = '<div class="sb-inline sb-inline-agents">
                <h3>' . sb_('Agent') . '</h3>
                <div id="conversation-agent" class="sb-select">
                    <p>' . sb_('None') . '</p>
                    <ul style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; border-radius: 6px;">  <!-- Fixed height & scroll -->
                        <li data-id="" data-value="">' . sb_('None') . '</li>';

    for ($i = 0; $i < count($agents); $i++) {
        $code .= '<li data-id="' . $agents[$i]['id'] . '">' . htmlspecialchars($agents[$i]['first_name'] . ' ' . $agents[$i]['last_name']) . '</li>';
    }
            ...
            <div class="sb-header">
                <div class="sb-admin-nav">
                    ...
                    <div>
                        ...
                        if($active_user['user_type']=='admin')
						{
							echo '<a href="https://tglevels.org/assign_agent" class="sb-icon-file"><span style="content: "\e908";" class="sb-icon-file"></span> <svg width="20" height="20" viewBox="0 0 24 24" fill="#24272a" style="position:absolute; left:20px; top:50%; transform:translateY(-50%); opacity:0.6;">
                    ...
                    <div class="sb-area-users">
                        <div class="sb-top-bar">
                            ...
                            <div>
                                <div class="sb-search-btn">
                                    <i class="sb-icon sb-icon-search"></i>
                                    <input type="text" autocomplete="false" placeholder="<?php sb_e('Search users ...') ?>" style="min-width:180px !important" />
                                </div>
                        ...
                        <div class="sb-tab">
                            <div class="sb-nav sb-scroll-area">
                                ...
                                <ul>
									<?php $a=1; if($a==1)
				                        { ?>
					<li id="tab-chat" class="sb-active0">
									...
									<li id="tab-admin" class="sb-active">
                                        <?php echo $disable_translations ? 'Tags' : 'Tags'//sb_('Admin') ?>
                                    </li>
									<?php $a=1; if($a==1)
				                      {?>
...
function sb_conversations_filter($cloud_active_apps) {
    ...
    $agents = sb_is_agent(false, true, true) && (sb_routing_is_active() || sb_get_multi_setting('agent-hide-conversations', 'agent-hide-conversations-active')) ? sb_db_get('SELECT id, first_name, last_name FROM sb_users WHERE user_type = "agent"', false) : [];
    $count = count($agents);
    if ($count > 1) {
        $code .= '<div class="sb-select"><p>' . sb_('All agents') . '</p><ul' . ($count > 8 ? ' class="sb-scroll-area"' : '') . '><li data-value="">' . sb_('All agents') . '</li>';
        for ($i = 0; $i < $count; $i++) {
            $code .= '<li data-value="' . $agents[$i]['id'] . '">' . $agents[$i]['first_name'] . ' ' . $agents[$i]['last_name'] . '</li>';
        }
        $code .= '</ul></div>';
    } else {
        $code .= '<div class="sb-select sb-hide"></div>';
