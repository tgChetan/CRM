...
if (!isset($_POST['function'])) {
    sb_api_error(sb_error('missing-function-name', 'api.php', 'Function name is required. Get it from the docs.'));
}
define('SB_API', true);
sb_process_api();
...
function sb_process_api() {
    ...
    $functions = [
        ...
        'set-rating' => ['conversation_id', 'user_id'],
        'get-rating' => ['user_id'],
    ...
    if (!isset($functions[$function_name])) {
        sb_api_error(sb_error('function-not-found', $function_name, 'Function ' . $function_name . ' not found. Check the function name.'), true, 404);
    }
    ...
    if (!$token) {
        sb_api_error(sb_error('token-not-found', $function_name, 'Admin token is required. Get it from ' . (sb_is_cloud() ? CLOUD_URL . '/account.' : 'Users > Your admin user profile box.')), true, 401);
    } else if (sb_is_cloud()) {
        require_once(SB_CLOUD_PATH . '/account/functions.php');
        cloud_api();
    } else if (!sb_api_security($_POST['token'])) {
        sb_api_error(sb_error('invalid-token', $function_name, 'Use a token of an administrator.'), true, 401);
    }
...
function sb_api_error($error, $die = true, $status_code = 400) {
