...
function sb_ajax_execute() {
    ...
    switch ($function) {
        ...
        case 'add-user':
            return sb_json_response(sb_add_user($_POST['settings'], sb_post('settings_extra', []), true, sb_is_agent()));
        ...
        case 'get-users':
            return sb_json_response(sb_get_users(sb_post('sorting', ['creation_time', 'DESC']), sb_post('user_types', []), sb_post('search', ''), sb_post('pagination'), sb_post('extra'), sb_post('users_id'), sb_post('department'), sb_post('tag'), sb_post('source'),sb_post('start_date', ''),sb_post('end_date', ''),sb_post('agent')));
        case 'get-new-users':
            return sb_json_response(sb_get_new_users($_POST['datetime'],$_POST['start_date'],$_POST['end_date']));
        case 'get-user-extra':
        ...
        case 'delete-user':
            return true;//sb_json_response(sb_delete_user($_POST['user_id']));
        case 'delete-users':
            return true;//sb_json_response(sb_delete_users($_POST['user_ids']));
        case 'update-user':
            return sb_json_response(sb_update_user($_POST['user_id'], sb_post('settings', []), sb_post('settings_extra', [])));
        case 'count-users':
        ...
        case 'update-conversation-extra':
            return sb_json_response(sb_update_conversation_extra($_POST['conversation_id'], sb_post('extra'), sb_post('extra_2'), sb_post('extra_3')));
        case 'queue':
        ...
        case 'login':
            return sb_json_response(sb_login(sb_post('email', ''), sb_post('password', ''), sb_post('user_id', ''), sb_post('user_token', sb_post('token', ''))));
        case 'logout':
        ...
        case 'send-message':
            return sb_send_message($_POST['user_id'], $_POST['conversation_id'], sb_post('message', ''), sb_post('attachments', []), sb_post('conversation_status_code', -1), sb_post('payload'), sb_post('queue'), sb_post('recipient_id'));
        ...
        case 'set-rating':
            return sb_json_response(sb_set_rating($_POST['conversation_id'], $_POST['user_id'], $_POST['rating'], sb_post('message'), sb_post('agent_id')));
        case 'get-rating':
        ...
        case 'delete-leads':
            return true;//sb_json_response(sb_delete_leads());
        case 'cron-jobs':
		...
		case 'direct-message':
    		...
    		$attachment_url = $_POST['attachment'] ?? null;
    		return sb_json_response(sb_direct_message($user_ids, $message, $attachment_url));
        ...
        case 'open-ai-message':
            return sb_json_response(sb_open_ai_message($_POST['message'], sb_post('max_tokens'), sb_post('model'), sb_post('conversation_id'), sb_post('extra'), sb_post('audio'), sb_post('attachments', []), sb_post('context')));
        ...
        case 'open-ai-qea-training':
            return sb_json_response(sb_open_ai_qea_training(sb_post('questions_answers', []), sb_post('language'), sb_post('reset'), sb_post('update_index')));
        ...
        case 'run-flow-on-load':
            return sb_json_response(sb_flows_run_on_load($_POST['message'], $_POST['conversation_id'], sb_post('language')));
        case 'open-ai-get-conversation-embeddings':
            return sb_json_response(sb_open_ai_embeddings_get_conversations());
        case 'open-ai-save-conversation-embeddings':
            return sb_json_response(sb_open_ai_embeddings_save_conversations($_POST['qea']));
        case 'open-ai-send-fallback-message':
            return sb_json_response(sb_open_ai_send_fallback_message($_POST['conversation_id']));
        ...
        case 'import-users':
            return sb_json_response(sb_import_users($_POST['file_url']));
        case 'otp':
            return sb_json_response(sb_otp($_POST['email'], sb_post('otp')));
        case 'shopify-cart-sync':
            return sb_json_response(sb_save_external_setting('shopify_cart_' . sb_get_active_user_id(), $_POST['cart']));
        case 'shopify-get-conversation-details':
            require_once(SB_CLOUD_PATH . '/account/functions.php');
            return sb_json_response(shopify_get_conversation_details(sb_post('shopify_id'), $_POST['user_id']));
        case 'shopify-get-product-link':
            require_once(SB_CLOUD_PATH . '/account/functions.php');
            return sb_json_response(shopify_get_product_link($_POST['product_id']));
        case 'shopify-get-products':
            require_once(SB_CLOUD_PATH . '/account/functions.php');
            return sb_json_response(shopify_get_products(sb_post('products_id'), sb_post('key'), sb_post('collection'), sb_post('search'), sb_post('pagination')));
        default:
            return '["error", "TG CRM Error [ajax.php]: No functions found with the given name."]';
    }
}
