 ...
 * ==========================================================
 *
 * Service Worker of TG CRM admin area
 *
*/

const SB_CACHE_NAME = 'sb-3-8-4';
...
if (typeof PusherPushNotifications != 'undefined') {
    PusherPushNotifications.onNotificationReceived = ({ pushEvent, payload }) => {
        ...
        pushEvent.waitUntil(clients.matchAll({
            type: 'window',
            includeUncontrolled: true
        }).then((clientList) => {
            for (var i = 0; i < clientList.length; i++) {
                if (clientList[i].url.split('?')[0] == sb_push_link.split('?')[0] && clientList[i].visibilityState === 'visible') {
                    return;
                }
            }
            pushEvent.waitUntil(self.registration.showNotification(payload.notification.title, { body: payload.notification.body, image: payload.data.image, icon: payload.notification.icon, data: payload.data, badge: payload.data.badge }));
        }));
    };
    self.addEventListener('notificationclick', function (event) {
        ...
        event.waitUntil(clients.matchAll({
            ...
            for (var i = 0; i < clientList.length; i++) {
                if (clientList[i].url.split('?')[0] == sb_push_link.split('?')[0]) {
                    clientList[i].postMessage({ 'conversation_id': sb_push_conversation_id, 'user_id': sb_push_user_conversation_id });
                    return clientList[i].focus();
                }
            }
            if (sb_push_link && clients.openWindow) {
                return clients.openWindow(sb_push_link);
            }
        }));
    });
}
