...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (startDateInput && endDateInput) {
        function onDateChange() {
            SBUsers.get(function(response) {
				SBUsers.sort();
                console.log("Users table refreshed with date filters", response);
            });
        }
    ...
    let end_date = $('#end_date').val();
        ...
        getRow: function (user) {
            if (user instanceof SBUser) {
                ...
                return `<tr data-user-id="${user.id}" data-user-type="${user.type}"><td><input type="checkbox" /></td><td class="sb-td-profile"><a class="sb-profile"><img loading="lazy" src="${user.image}" /><span>${user.name}</span></a></td>${code}<td class="sb-td-email">${user.get('email')}</td><td class="sb-td-ut">${sb_(user.type)}</td><td class="sb-td-tag">${user.details.tags?user.details.tags:'Free'}</td><td>${SBF.beautifyTime(user.get('last_activity'), true)}</td><td>${SBF.beautifyTime(user.get('creation_time'))}</td></tr>`;
            } else {
                        ...
                        if (response.get('source') == 'wa') {
                            ...
                            if (datetime) {
                                if ((new Date()) - (new Date(datetime.get('creation_time').replace(' ', 'T'))) > 86400000) {
                                    infoBottom(sb_('You can\'t send a WhatsApp message more than 24 hours after the user\'s last message—use a template instead.') + '<i id="sb-whatsapp-alert-btn" class="sb-icon-social-wa"</i>', 'info');
                                }
                            }
                        }
    ...
    var SBProfile = {
        ...
        show: function (user_id) {
            ...
            activeUser().update((response) => {
				...
				profile_box.find('#user_communities').html(response.communities);
    ...
    */
    ...
    $(document).ready(function () {
        ...
        // Updates and apps
        let messages = [sb_('Please go to Settings > TG CRM.'), `${sb_('Your license key is expired. Please purchase a new license')} <a href="https://tglevels.in/shop/{R}" target="_blank">${sb_('here')}</a>.`];
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { sb: 'TG CRM', slack: 'Slack', dialogflow: 'Artificial Intelligence', tickets: 'Tickets', woocommerce: 'Woocommerce', ump: 'Ultimate Membership Pro', perfex: 'Perfex', whmcs: 'WHMCS', aecommerce: 'Active eCommerce', messenger: 'Messenger', whatsapp: 'WhatsApp', armember: 'ARMember', telegram: 'Telegram', viber: 'Viber', line: 'LINE', wechat: 'WeChat', zalo: 'Zalo', twitter: 'Twitter', zendesk: 'Zendesk', martfury: 'Martfury', opencart: 'OpenCart', zalo: 'Zalo' };
                let updates = false;
        ...
        $(admin).on('click', '.sb-apps > div:not(.sb-disabled)', function () {
            ...
            box.find('.sb-btn-app-puchase').attr('href', 'https://tglevels.in/shop/' + app_name + ga);
            box.find('.sb-btn-app-details').attr('href', (is_cloud ? WEBSITE_URL : 'https://tglevels.in/') + app_name + ga);
            box.sbShowLightbox();
        });
...
box.find('.sb-attachments > div').each(function () {
    let attachment = {
        name: $(this).attr('data-name'),
        value: $(this).attr('data-value'),
        size: $(this).attr('data-size') || null
    };
    attachments.push(attachment);
        ...
        */
        ...
        $(settings_area).on('click', '#slack-button a', () => {
            window.open('https://tglevels.in/synch/?service=slack&plugin_url=' + SB_URL + cloudURL());
            return false;
        });
        ...
        if (!SB_ADMIN_SETTINGS.cloud) {
            $(settings_area).on('click', '#messenger-sync-btn a', () => {
                window.open('https://tglevels.in/synch/?service=messenger&plugin_url=' + SB_URL + cloudURL());
                return false;
            });
        }
