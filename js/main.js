...
(function ($) {
    ...
    */
    ...
    var SBF = {
        ...
        error: function (message, function_name) {
            ...
            throw new Error(is_full_error ? message : `TG CRM Error [${function_name}]: ${message}.`);
        },
        ...
        // Remove the TG CRM syntax from a string
    ...
    */
    ...
    var SBChat = {
		...
		addUserAndLogin: function (onSuccess = false, lead = false) {
    const scriptTag = document.getElementById('sbinit');
    const userId = scriptTag.getAttribute('data-user-id');
    const fullName = scriptTag.getAttribute('data-full-name');
    const userPhone = scriptTag.getAttribute('data-phone');
	const userEmail = scriptTag.getAttribute('data-email');
	
    let settings = (typeof SB_DEFAULT_USER !== 'undefined' && SB_DEFAULT_USER) ? SB_DEFAULT_USER : {};

    settings.user_type = lead ? 'lead' : 'visitor';
    settings.user_id = userId ? userId : 1;
    settings.first_name = fullName ? fullName : "USER";
    settings.phone = userPhone;
    settings.email = userEmail;
	settings.password = 'password';
    settings.extra = settings.extra || {};
    settings.extra.phone = userPhone;
    ...
    SBF.ajax({
        ...
        if (SBF.errorValidation(response)) {
            if (response[1] == 'duplicate-email' || response[1] == 'duplicate-phone') {
                ...
                // ✅ Instead of retrying addUserAndLogin, call login directly
                return SBF.ajax({
                    function: 'login',
                    ...
                    if (Array.isArray(loginResponse)) {
                        SBF.loginCookie(loginResponse[1]);
                        activeUser(new SBUser(loginResponse[0]));
                        SBPusher.start();
                        if (!SBPusher.active) {
                            SBChat.automations.runAll();
                        }
                        if (onSuccess) onSuccess(loginResponse);
                    } else {
                        console.warn('Login failed after duplicate:', loginResponse);
                    }
                });
            }
            console.warn('Add user failed:', response);
        } else {
    ...
    function initialize() {
        ...
        if (main.length && typeof SB_AJAX_URL != ND) {
            ...
            if (SBF.cookie('sb-check') != 'ok') {
                ...
                console.warn('TG CRM: cookies not available.');
            } else {
		...
		// Prevent double binding globally
if (!window.sbAttachmentHandlerBound) {
    window.sbAttachmentHandlerBound = true;
    $(document).on('click', '.sb-btn-attachment', function (e) {
        e.stopImmediatePropagation(); // stop any duplicate handlers from firing
        if (!SBChat.is_busy) {
            const editor = $(this).closest('.sb-editor');
            editor.find('.sb-upload-files').val('').click();
        }
    });
}
